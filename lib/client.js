...
class Client extends EventEmitter {
  ...
  onServerSocketConnect(sock) {
    sock.on('recv', (data) => {
      console.log('client server recv', data);
      switch (data.action) {
        case 'server.reg': {
  ...
  onClientSocketConnect(sock, port) {
    sock.on('recv', (data) => {
      console.log('client client recv', data);
      switch (data.action) {
        case 'client.reg': {
      ...
      Object.keys(this.remoteRooms).forEach((nsp) => {
        ...
        Object.keys(rn).forEach((room) => {
          ...
          Object.keys(rr).forEach((id) => {
            if (rr[id] === port) {
              console.log('close delete room member', room, id);
              delete rr[id];
            }
            this.emit('client.del', id, room, nsp);
          });
          if (!Object.keys(rr).length) {
            console.log('close delete room', room);
            delete rn[room];
          }
        });
        if (!Object.keys(rn).length) {
          console.log('close delete nsp', nsp);
  ...
  rpcReq(id, key, args, nsp, uuid) {
    let rn = this.remoteRooms[nsp];
    if (!rn) {
      console.log('no such remote nsp');
      return;
    }
    let port = (rn[id] || {})[id];
    if (!port) {
      console.log('no such remote id', id);
      return;
    }
    let data = {
